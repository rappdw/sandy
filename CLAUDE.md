...
**TOCTOU — never stop a session this run did not create.** `--provision` reads `sandy.provisioned_at` back off the container immediately after its own `--start` returns (the value it owns), then re-reads it immediately before calling `--stop`. A mismatch — a different container id, a missing label, or a different stamp — means some other session now occupies the workspace (e.g. a concurrent plain `sandy --start` raced it and D6 idempotency made *this* run's own `--start` call a no-op against *that* session), and `--provision` leaves it running rather than risk tearing down a session it did not create: exit `0`, `--stop` is never called.
