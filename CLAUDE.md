...
| `CASESUB` | a `case` **pattern** inside a multi-line `$( )` | same scanner flaw as `APOSCS`, different token: the unbalanced `)` that terminates a case pattern closes the substitution early. §119 carried this for weeks — green in CI, aborting the whole macOS run after 1591 passing checks, before the sections after it ever ran. Fix: hoist the `case` out, or use the POSIX leading-paren form `(pattern)` |
