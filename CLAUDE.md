...
Default `0` — unset means no directories are created and no mounts/env vars are added (zero launch diff). Guarded by `run-tests.sh §86` (structure) and `test/acceptance-handoff-mailbox.sh` (real-Docker mount/mode behavior, invoked as `run-integration-tests.sh §23`).
