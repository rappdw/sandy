...
Fixes: `SRCSUB` → extract-then-`eval`; `PYBACK` → a **quoted** heredoc (`python3 - arg <<'PY'`); `APOSCS`/`APOSQ` → reword the comment; `GREPM` → `grep -m1`; `GNUBIN` → resolve `timeout`→`gtimeout` (or drop it), `shasum -a 256`, `sed -i ''`/`-i.bak`, a `stat -f` fallback.
