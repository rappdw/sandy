...
**Do not anchor a JSON assertion on a field's NEIGHBOURS.** `sed -n 's/.*"name":"X"[^}]*"relay":{...'` reads naturally and is a tripwire: the character class cannot span a nested object, so the day someone inserts one between the two fields, the check fails against an emitter that is entirely correct. It has cost two rounds — §123 broke when `handoff{}` landed ahead of `relay{}`, and §124(8) broke on its own `dirs{}`. Portable per-object splitting is not worth it (BSD `sed` will not put a newline in a replacement; BWK `awk` takes only a single-character `RS`), so **separate the fixtures instead of the output**: read each case from a `$SANDY_HOME` containing only that sandbox, and the document is unambiguous without carving.
