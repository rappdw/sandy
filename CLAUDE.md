...
| `GNUBIN` | a GNU-only binary or flag in **host-side** shell — `timeout`, `sha256sum`/`md5sum`, `realpath`, `date -d`, `grep -P`, `stat -c` with no BSD fallback, `sed -i` with no suffix, `xargs -r`, `find -printf`, `sleep infinity` | macOS ships no `timeout` (homebrew calls it `gtimeout`): exit 127, swallowed by a `|| true`, and the assertion then grepped `"timeout: command not found"` for real output. Three separate debugging rounds in one session (§108, §109, `acceptance-handoff-dirs.sh` E8) |
