...
**Three of the four lanes were REMOVED in 2.2.0 (#352).** `inbox`, `outbox` and `peer` are gone, with their `SANDY_HANDOFF_{INBOX,OUTBOX,PEER}` exports — a feature manifest names its own directories via `mounts`, and every lane was measured empty across the fleet before removal. **Only `handoff/relay` survives**, and only until #353 moves relay state to a producer-agnostic directory; that move is what finally retires `SANDY_HANDOFF_DIRS` and the tree. `_sandy_handoff_classify` now inspects `relay` alone — leaving it iterating four names would have made every sandbox report `handoff.state: "missing"` the moment the first lane went, which is why the classifier and the mounts had to change together. The paragraphs below describe the pre-2.2.0 shape and are kept for the reasoning, not as current behaviour.

**On by default since 1.10.0.** `SANDY_HANDOFF_DIRS` (passive-safe, default `1`; was `0`) mounts four children of `$SANDBOX_DIR/handoff/` into every sandbox on every launch, idempotently: `outbox` rw at `~/.handoff/outbox`, `inbox` **`:ro`** at `~/.handoff/inbox`, `peer` **`:ro`** at `~/.handoff/peer` (a second host-written inbound directory, added in 1.10.0), and `relay` rw at `~/.handoff/relay` (relay state + `supervisor.log`). The point of the default is that a host-side process can be pointed at any sandbox without a per-sandbox enabling step that has to be remembered, provisioned and verified. `SANDY_HANDOFF_DIRS=0` is the opt-out, honoured from the same tiers the enabling value always was (env, `~/.sandy/config`, workspace `.sandy/config`): nothing is mounted, no env vars are added, `~/.handoff` does not exist in the container. The `:ro` mount flag is the actual boundary — the container process runs as the host uid and owns all four, so against a plain rw mount an in-container `chmod` would succeed (`chown` would not; the agent runs unprivileged via gosu with an empty capability set).

**Why both values stay passive, and why the default flip is not a tier change.** A directory confers no reach on its own: nothing lands in `inbox`/`peer` unless something on the host writes there, and nothing leaves `outbox` unless something on the host reads it. `=1` is therefore what a workspace could always set without approval, and `=0` only tightens — neither is a weakening value for `_sandy_passive_value_privileged`. The two things that *do* confer reach are unchanged and privileged: `SANDY_HANDOFF_RELAY` (off unless set) and the `crossSessionInbound` pin (`accept` only when a relay is set and will start). A configured relay forces the key to `1` over an explicit opt-out, with an info line saying so, because the relay cannot run without its mounts and a relay that cannot start fails the launch (criterion 7).

**Operator-side marker**: a `$SANDBOX_DIR/.handoff-enabled` file (contents ignored) forces the tree **on for that sandbox regardless of any config** — with the default on, it is the inverse of the opt-out rather than an enable, and a no-op when nothing opts out. It exists because a workspace `.sandy/config` **travels with the repository** — clone a repo that opts out and every sandbox of it is out, on machines nobody consulted. The marker is per-machine, lives in state a repo cannot carry, and is privileged by construction of *where* it lives (`$SANDY_HOME`), so it wins over a passive config exactly as `agent-args.<agent>` does; "off everywhere, on for these" is one host-config line plus a `touch` per sandbox. It is at the sandbox **top level, specifically not under `claude/`** (which is mounted rw at `~/.claude`, so a marker there would be one the *agent* could create for itself). Nothing bind-mounts the sandbox top level; §97 asserts that.

**Directory presence is deliberately NOT a signal, and consumers verify the mounts.** The host directories are created on **every** launch regardless of the value (since 1.7.0), which makes *presence carries no information* true by **construction** rather than convention — a stray `mkdir`, a restored backup, or an `rsync -a` can neither enable nor defeat anything. Unmounted they are inert. A consumer that needs to know whether a sandbox has the tree checks the mounts: inside, `test -d ~/.handoff/inbox ~/.handoff/outbox ~/.handoff/peer` (and a write to `inbox`/`peer` failing with EROFS); outside, `docker inspect -f '{{range .Mounts}}{{.Destination}} {{.RW}}{{"\n"}}{{end}}' <container>` filtered on `/home/sandy/.handoff/`. Never the directories, never the marker.
