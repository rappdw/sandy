...
**Do not anchor a JSON assertion on a field's NEIGHBOURS.** `sed -n 's/.*"name":"X"[^}]*"relay":{...'` reads naturally and is a tripwire: the character class cannot span a nested object, so the day someone inserts one between the two fields, the check fails against an emitter that is entirely correct. It has cost **three** rounds — §123 broke when `handoff{}` landed ahead of `relay{}`, §124(8) broke on its own `dirs{}`, and `acceptance-handoff-dirs.sh` Phase F spent a maintainer run reporting `relay.state=started` as FAILING while `--print-state` was emitting exactly that. **`run-tests.sh §88b` now ratchets against it** across the whole test tree (comment lines exempt, so the rule can be written down). Portable per-object splitting is not worth it (BSD `sed` will not put a newline in a replacement; BWK `awk` takes only a single-character `RS`), so **separate the fixtures instead of the output**: read each case from a `$SANDY_HOME` containing only that sandbox, and the document is unambiguous without carving. Where the fixtures cannot be separated — an acceptance harness against a live home — slice the one object out first with `index`/`substr` in `awk` (no regex, so no BRE-vs-ERE question, and BWK-safe), then assert on the slice. **Write that program on one line**: a multi-line single-quoted program argument desynchronizes the portability lint's quote tracking and produces findings against unrelated comments below it.
...
| `VARMB` | an **unbraced** `$name` immediately followed by a non-ASCII character (`"Probing $host…"`) | bash 3.2 reads the bytes of the `…` as part of the **name**, looks up a variable that does not exist, and `set -u` aborts: `_rsy_host�: unbound variable`. bash 5 ends the name at the first non-ASCII byte, so CI never sees it. The first macOS run of `sandy --rsync` died on it (#374). Heredoc bodies are skipped (container-side, bash 5) |
...
| `CASESUB` | a `case` **pattern** inside a multi-line `$( )` | same scanner flaw as `APOSCS`, different token: the unbalanced `)` that terminates a case pattern closes the substitution early. §119 carried this for weeks — green in CI, aborting the whole macOS run after 1591 passing checks, before the sections after it ever ran. Then **§126 carried a second one from 1.12.0 through 1.13.1** — four releases, aborting at 1720 checks, so §127/§128/§129 never ran on macOS at all. It needed **two** blind spots to hide: the span opener `out="$( {` matched none of the three opener rules (so nothing in that function was scanned, for any detector), and the matcher was anchored to the start of a line while that `case` sat mid-statement after `then command(){`. Both are closed, each with a fixture, plus a negative control asserting the `(pattern)` fix does **not** fire. Fix: hoist the `case` out, or use the POSIX leading-paren form `(pattern)` |
| `GNUBIN` | a GNU-only binary or flag in **host-side** shell — `timeout`, `sha256sum`/`md5sum`, `realpath`, `date -d`, `grep -P`, `stat -c` with no BSD fallback, `sed -i` with no suffix, `xargs -r`, `find -printf`, `sleep infinity`, and **`\|` alternation in a `sed` BRE** | macOS ships no `timeout` (homebrew calls it `gtimeout`): exit 127, swallowed by a `|| true`, and the assertion then grepped `"timeout: command not found"` for real output. Three separate debugging rounds in one session (§108, §109, `acceptance-handoff-dirs.sh` E8) |

**`\|` is a GNU extension to BRE, and `sed` and `grep` diverge on it.** POSIX basic regular expressions have no alternation at all, so BSD `sed` matches a **literal pipe**: the expression never fires, prints nothing, and exits `0`. That is §127(6-9) — four checks that passed in CI and failed on the first macOS run they ever got, with no error to read. Use POSIX ERE instead (`sed -E -n 's/...(\[[^]]*\]|null).*/\1/p'`); both seds accept `-E`. **`grep` is different and is deliberately NOT linted**: the same macOS run passed §109(2), whose `grep -q "sandy own state\|sandy.s own state"` has no fallback and whose first alternative does not occur in the file — it can only have passed if BSD grep honoured the alternation. Flagging grep too would have produced 12 findings against lines that demonstrably work.

Fixes: `SRCSUB` → extract-then-`eval`; `VARMB` → brace it, `${name}…`; `PYBACK` → a **quoted** heredoc (`python3 - arg <<'PY'`); `APOSCS`/`APOSQ` → reword the comment; `GREPM` → `grep -m1`; `GNUBIN` → resolve `timeout`→`gtimeout` (or drop it), `shasum -a 256`, `sed -i ''`/`-i.bak`, a `stat -f` fallback.

**`GNUBIN` scope, because it is the whole difficulty of that detector**: only *host-side* shell is a hazard — the container is Linux, where GNU tools are correct. So heredoc **bodies** are skipped (container-side scripts or another language), as is `templates/user-setup.sh.tmpl`. It also stands down for a file that ships its own shim (`timeout() { … }`), for a same-line BSD fallback chain (`shasum -a 256 … || sha256sum`), and for a branch guarded by `command -v NAME` / `NAME --version` within 20 lines above. Container-side code that is **not** in a heredoc — a single-quoted multi-line argument, which no heuristic can tell from host shell — needs an explicit `# lint-bash32: allow GNUBIN` on the line or the line above. Generated mirrors keep coverage where it counts: `doctor.sh` is host-side and is scanned as its own file even though its heredoc body inside `sandy` is skipped.
...
Also note (from real macOS runs): new test sections must be BSD/bash-3.2-safe or they pass CI and fail the maintainer — watch `\$`-literal-in-awk, `wc -l` leading whitespace, and empty-array expansion under `set -u`. **`mktemp -d` returns a symlink on macOS** (`/var/folders/…` → `/private/var/folders/…`), so a fixture that uses it as `$HOME` and then relies on sandy's `$HOME`-relative workspace mapping silently takes the fallback path: sandy canonicalizes the workspace with `pwd -P` but takes `$HOME` as given, the prefix stops matching, and `-w` becomes a host path. Cost §120(4)/§120(14), macOS-only, invisible in CI where `/tmp` is not a symlink. Canonicalize at creation: `D="$(cd "$(mktemp -d)" && pwd -P)"`. When extracting a section to validate locally, replicate `set -euo pipefail` **and** the ERR trap, or the harness cannot see an unguarded non-zero exit.
...
**An entry may be WITHDRAWN in any release** — struck through and marked *kept*. Withdrawal is permitted in a minor because it **strictly reduces** what a consumer must plan for: it retracts a threat rather than creating one, which is the opposite of what the add-only rule guards against. The rule was silent on this until 2.2.0, when `relay{}` needed it: its announced replacement was never built, and the premise for replacing it disappeared along with the other relay producers.
...
- **Claude** (`SANDY_CLAUDE_AUTH=auto|api_key|oauth|profile`) **An expired host token never prompts where nobody can answer (#261, 1.12.0).** Sandy refreshes an expiring OAuth credential by running `claude auth login`, which opens a browser *and* waits on "Paste code here". `can_open_browser` used to return true on macOS unconditionally — "the user is local" — which is false in the **detached daemon supervisor**, whose stdin is `/dev/null`. The prompt could never be answered, `--start` waited out its full 600s readiness timeout, and because the supervisor had already taken the workspace lock, every later launch in that workspace failed `Another sandy is already running (pid N)`. One expired token poisoned a whole test run, and the real cause sat 300 lines above the first visible failure. The refresh is now skipped in the supervisor, in headless `-p`, and whenever stdin is not a TTY. **And then the launch REFUSES rather than proceeding**: dropping only the prompt would continue with an expired credential, Claude Code would come up on its own `/login`, and with nobody attached the session is **inert** — container up, `tmux has-session` fine, `--start` reporting ready, messages queueing forever. That is the same shape as #151, the custom-API-key modal and #256's theme picker, and it would trade a loud 600s failure for a silent permanent one. So those contexts drop the `.fatal` marker (client fails in ~1s, exit `6`) and exit, with the EXIT trap releasing the lock so the failure does not poison the next launch. An **interactive foreground** launch is deliberately untouched — a human is attached and `/login` works there. Guarded by §125, whose probes pin `uname` to Darwin: `can_open_browser`'s last line is the macOS test, so on Linux it returns false for everyone and a regression guard that does not exercise the macOS branch would have passed with the bug restored. — `auto` probes `CLAUDE_CODE_OAUTH_TOKEN`, then the host OAuth credentials (`~/.claude/.credentials.json`, or the macOS **login Keychain** entry `Claude Code-credentials`), then `ANTHROPIC_API_KEY`. **At most one Claude credential reaches the container.** Claude Code resolves an environment API key ahead of both, so sandy suppresses `ANTHROPIC_API_KEY` whenever either OAuth form is in play — otherwise the key silently bills per-use, or, if it has never been approved in that sandbox's `.claude.json`, Claude Code parks on its custom-API-key startup modal. That modal is the dangerous one in daemon mode: the container is up and `tmux has-session` succeeds, so **`--start` reports ready (exit 0) for a session that is inert** — it accepts cross-session messages, queues them, and never runs them. It cost three rounds of debugging disguised as "`accept` does not deliver". **`SANDY_CLAUDE_AUTH=api_key` is how you ask for the key anyway** — it withholds the OAuth file *and* a long-lived token, so one revocable project key is the only Claude credential in the container. Without it, suppression left no way to choose per-use billing on a host that has OAuth except `SANDY_SUSPICIOUS=1`, which is a package deal (refresh-token strip, connectors off, egress strict), not a billing choice. `oauth` forces the account credential in even when a key is present, so it is the approval-gated value; `api_key` *reduces* what is in the box and stays passive-safe. **`profile`** (1.11.0) uses an Anthropic Console profile from `ant auth login` — the only route to a workspace-bound entitlement such as Claude Mythos. Measured on 2.1.263: Claude Code ranks a `user_oauth` profile **below** a `/login` credential and below every env credential, so profile mode withholds all three (the one-credential rule again) and copies **only the selected profile** — a config dir can hold an `org:admin` one. The mount is rw (Claude Code refreshes the token at startup) but ephemeral. `ANTHROPIC_PROFILE` selects the profile and is **privileged**: it chooses which token enters the box. Guarded by §117.
- **Gemini** — `GEMINI_API_KEY`, host `~/.gemini/tokens.json`, host gcloud ADC.
- **Codex** — `OPENAI_API_KEY` (materialized as an ephemeral `auth.json`, mounted `:ro`, because codex 0.139+ no longer reads the env var for first-party auth), then `$SANDBOX_DIR/codex/auth.json` if this sandbox has one, then host `~/.codex/auth.json`. The host copy is **seeded into the rw sandbox, not overlaid `:ro`** — so `codex login`, `codex logout` and in-session refresh work inside the container and **persist for that sandbox**, and the sandbox's own copy then wins over the host's on later launches (otherwise a relaunch would clobber a fresh login with the host's stale token). The host file is only ever read, never written back. Mirrors grok, whose interactive login already persists in its rw `~/.grok` mount. **The api_key path is the exception**: it stays an ephemeral `:ro` overlay, so in-container `codex login` is refused there — the value comes from the environment and is re-supplied every launch, so there is nothing to persist, and writing a raw API key to disk in the sandbox is a larger change than persisting an OAuth session. Unset `OPENAI_API_KEY` or set `SANDY_CODEX_AUTH=oauth` to log in interactively. **In-container, that login must be `codex login --device-auth`.** Plain `codex login` binds a callback server on the *container's* `localhost:1455`, which a host browser can never reach — and under the egress proxy the agent is on an `--internal` sidecar with no inbound path at all; codex's own output says to use `--device-auth` on a remote or headless machine. The device flow binds no port: it prints `https://auth.openai.com/codex/device` plus a user code and polls for the token, so it works unchanged in the sandbox and persists to the rw `$SANDBOX_DIR/codex/auth.json`. Strict egress already covers it via `*.openai.com`.
...
**Launch specifics.** Gemini gets `GEMINI_SANDBOX=false`. Codex gets `--sandbox danger-full-access` plus `sandbox_mode` in `config.toml` (its Landlock sandbox does not nest in Docker; sandy provides the outer isolation), a seeded `[notice]` block, and a `trust_level = "trusted"` entry appended by `user-setup.sh`. **The `config.toml` seed is first-run only, so `sandbox_mode` is separately repaired by VALUE every launch** — the seed gate greps for the key, not the value, and since `~/.codex` went rw (#238) an in-session edit by the user *or by codex* persists forever. A wrong mode makes codex nest its Landlock sandbox inside the container, commands fail to spawn, and codex falls back to its approval path — which reads like a sandy fault and is not one. The repair rewrites one top-level line, warns, preserves everything else, and deliberately leaves a `[profiles.…]`-scoped value alone. Guarded by §118. Grok headless adds `--no-auto-update`.
...
**A fresh sandbox also seeds `hasCompletedOnboarding` + `theme`.** With no host `~/.claude.json` to copy from — every clean CI runner, and any user who has never run claude outside sandy — Claude Code opens its first-run **theme picker** and waits. Interactively that is one keypress; under `--start` nobody is attached, so the container is up, `tmux has-session` succeeds, the client reports **ready (exit 0)**, and the agent sits on a dialog forever. Same class as the auto-mode nudge and the custom-API-key modal: a first-run prompt that makes a daemon session inert while every structural check passes. `hasCompletedOnboarding` is re-merged every launch so pre-fix sandboxes heal; `theme` is seeded once so a later `/theme` survives. Caught by `acceptance-uds-delivery.sh` in CI, whose pane dump showed the picker where the agent should have been. Guarded by §122.
...
- **below the floor** → **hard error, sandy refuses to launch**, printing the recreation command, and dropping the `.fatal` marker so a `--start` client fails in ~1s (exit `6`) instead of polling out its 600s readiness timeout
- **unknown/invalid** (pre-0.10.1, or unreadable) → warn only (fail-open on uncertainty, fail-closed on proof)
- **at/after** → silent

The floor moved to `2.0.0` for the container-user rename (#248): `/home/claude` is baked into persistent sandbox state — venv shebangs, `GOPATH`, `PYTHONUSERBASE`, npm and cargo metadata — none of which a relaunch rewrites, so a `1.x` sandbox under a `2.x` sandy breaks quietly rather than loudly. **Every pre-2.0 sandbox is refused**, which is the whole population; that is the cost the major buys.

**Fix: `sandy --reset-sandbox --workspace <path> --keep-history`, not `rm -rf`.** The two are not equivalent and the difference is not recoverable: `--reset-sandbox` preserves `WORKSPACE.json` lineage, `.handoff-enabled`, `relay-bin/`, `agent-args.*`, and — with `--keep-history` — `claude/projects/`, which holds session transcripts and auto-memory that **nothing recreates**. `sandy --reset-sandbox --all --keep-history --yes` is the fleet form and the one command a 2.0 upgrade needs. The earlier `rm -rf ~/.sandy/sandboxes/<name>` advice predates all of that preserved state and is now actively destructive.

**Promise: a sandbox created by any `X.y` sandy works with any later `X.y` sandy.** Therefore **within a major, `SANDY_SANDBOX_MIN_COMPAT` must never advance above `X.0.0`** — a layout change that would break same-major sandboxes is the next major. Guarded by `run-tests.sh §51/§60` plus the frozen `test/fixtures/frozen-sandbox-1.0/` snapshot, which since 2.0 proves the **refusal** rather than acceptance (read its README before "fixing" a §60 failure).
...
- **`--print-version`** — `{schema_version, version, commit, full_version}`. Exists because `--print-schema`'s own `sandy.version` is the payload a consumer would be caching (chicken-and-egg). **Cache on `full_version`, not `version`**: on a dev channel `version` stays `"1.7.0-dev"` across every commit, so a cache keyed on it never invalidates across exactly the upgrades that matter. `commit` is `""`, not `null`, when unknown. **And a stripped version is not evidence a feature exists**: `X.Y.Z-dev` compares equal to `X.Y.Z` under any `(\d+)\.(\d+)\.(\d+)` match *and* under `_ver_lt`, so a `>= X.Y.Z` gate is satisfied by every build on that dev line, including commits predating the feature. Probe the **capability** — a key in `--print-schema`'s tier lists, a field in `--print-state` — not the number. Pre-1.7.0 sandy forwards unrecognized flags to the agent rather than erroring, so confirm 1.7.0+ via `--version` first — matching its full `sandy <full_version>` string, never a numeric triple parsed out of it, since that strip walks into the same trap and the cost there is an unintended container launch.
...
**`agents` — the resolved agent list (1.13.0).** `SANDY_AGENT` settles from **four** sources (env, host `~/.sandy/config`, workspace `.sandy/config`, the `--agent` flag) plus the `all` alias, so a consumer reading a workspace's own config cannot know what will launch — **a workspace naming nothing may still launch as anything**. Only the launch knows, so the launch records it, as a JSON array in `SANDY_AGENT` order (which is also pane order). `--print-state` reads it back per sandbox.
...
**The container user is `sandy` and its home is `/home/sandy` (2.0.0, #248)** — it was `claude`, which was agent-specific naming in a five-agent sandbox. This is exactly why the rename had to be a major: `/home/claude` is baked into persistent sandbox state (venv shebangs, `GOPATH`, `PYTHONUSERBASE`, npm/cargo metadata), so it forces `SANDY_SANDBOX_MIN_COMPAT` above the `1.0.0` ceiling the `1.x` forward-compat promise imposed, and it moves published consumer contracts (`~/.handoff/*`, `/ss`, `SANDY_WORKSPACE`). Every pre-2.0 sandbox is therefore refused at launch, with the recreation command printed; `--reset-sandbox --all --keep-history` is the fleet form.

**The rename is three spellings, not one, and a path renamed beside a username reads as done when it is not.** The sweep moved `HOME=/home/sandy` on the same line as `su -s /bin/bash claude` and left the username; nothing static ran it, so the agent image failed at `docker build` in CI and nowhere else. §140(9b) now enumerates the ways a username reaches a command — `su`, `gosu`, `chown`, `--chown=`, `USER`, `adduser`, `useradd`, `-u` — rather than the two that were known about. **And a doc sweep is not a rename**: a `claude`→`sandy` pass rewrites prose *describing* the old state into prose that contradicts itself, which is how this paragraph read for two commits.

**`sandy --exec` — a shell in the running container, and why the hand-rolled form is wrong.** `docker exec -u sandy` runs as **uid 1001**, not the host uid: the image does `useradd -u 1001 sandy`, sandy bind-mounts a generated `/etc/passwd` carrying the host uid, and docker resolves `-u <name>` against the **image** filesystem rather than the runtime mount. Confirmed on a real container (`docker exec -u sandy <c> id` → `uid=1001`). The visible tell is a prompt reading `I have no name!`; the damaging part is writes landing as the wrong owner on a workspace mount owned by the host user. `--exec` therefore uses numeric `-u $(id -u):$(id -g)`, sets `-w` to the container-side workspace path, and passes `HOME` explicitly — docker only derives HOME when it can resolve the user, and as root `HOME=/root` is on the read-only rootfs, which is why an in-container `codex logout` returned `Read-only file system (os error 30)`. Resolution is daemon-label first, then a **fully anchored** foreground name match (a workspace named `proxy` yields agent `sandy-proxy-<hash>` beside sidecar `sandy-proxy-proxy-<hash>`). Exit `4` = no running container; otherwise the command's own status. `--dry-run` prints the command and runs nothing. Guarded by §120, which asserts the **real** argv as well as the `--dry-run` rendering — they are separate render paths and a first cut of the guards missed a mutation that changed only one.
...
- **`--rsync <dest_host> [--workspace P] [--dest-workspace P] [--dest-sandy-home P] [--dry-run] [--yes]`** (2.4.0, #374) — **copies** this workspace's sandbox to another host over `ssh` + `rsync`; the source is never modified. Filesystem-only, like `--reset-sandbox`. A hand `rsync` gets four things wrong, each losing state nothing recreates: (1) the directory is **named from the workspace path** (`<basename>-<sha8>`), so a different absolute path means a different directory on the destination; (2) **`.claude.json` lives beside it**, at `$SANDY_HOME/sandboxes/<name>.claude.json`, so a directory copy drops it; (3) if the workspace moves **relative to `$HOME`**, the container path moves, and Claude Code keys session history and auto-memory by it — `claude/projects/<path with every non-alphanumeric → ->`, measured from the 2.1.278 bundle (`replace(/[^a-zA-Z0-9]/g,"-")`); (4) until the first launch there, `WORKSPACE.json` names a path that does not exist on the destination, which is exactly `--remove-sandbox --orphans`' deletion criterion. It fixes all four: renames the directory to the name the **destination** computes from its own `pwd -P`, renames the history directory, rewrites the `.claude.json` projects key, and rewrites `WORKSPACE.json`'s path while keeping its lineage. Decisions (#374): the destination workspace **defaults to the same `$HOME`-relative path** and must already exist there (only the destination can canonicalize it); **credential-bearing files are copied and each is named in the plan**; on a **CPU-architecture mismatch** `cargo/ go/ npm-global/ pip/ uv/ venv/` are skipped (`aarch64` = `arm64`). `venv/` is also skipped whenever the container path changes (its scripts hardcode it). Never copied: `claude/sessions/` (per-process messaging keys), `sandy-session.json`, `relay-state/`, `agent-args-composed/`, `proxy.log`. Refuses on a live lock or a running `sandy-<name>` container — except under `--dry-run`, which only reads and so still prints the plan, then says a real run would refuse (the case you hit first when sandy is developed inside sandy: the workspace you want to copy is the one your own session holds) — refuses to overwrite a sandbox already on the destination, and refuses a host that starts with `-` or contains `:` — a leading `-` reaches `ssh` as an **option** (`-oProxyCommand=` is command execution) and `:` is rsync's host/path separator. The destination `SANDY_HOME` is checked against a plain-path charset rather than escaped, because rsync's `--protect-args` is not in every rsync (macOS ships openrsync). One `ssh` round trip probes the destination; exit `0` only after re-checking the copy there (C1). The name is computed by `_sandy_slug_for`, which the launch path computes inline; §154(1) holds them equal, and §154(2) does the same for the container path, including the `/home/u` vs `/home/ux` prefix trap. Guarded by §154, every check mutation-tested.
- **`--provision [--workspace P] [--dry-run] [--yes]`** — creates a sandbox non-interactively **via the real launch path**: `SANDY_PROVISION=1 "$0" --start` then `"$0" --stop` (DEC-U2 composition; needs Docker, unlike the filesystem-only members). Deliberately **not** a flag that manufactures the directories — hand-created state that no container ever mounted *looks wired and is not*, and a second code path would be free to drift. Completion = `--start` 0 then `--stop` 0; `--stop` 4 after a successful start is also success (the launch already completed), `--stop` 5 is a failure. Two liveness guards (workspace lock, then a running daemon container per D9) make it a **safe no-op, exit 0** against a live session; it is always re-runnable and never skipped merely because the directory exists. TOCTOU: it mints a random `SANDY_PROVISION_ID` per run (separate from the session nonce, which is the attestation trust root) and the supervisor stamps it as `sandy.provision_id`, so both reads prove *ownership*, not mere stability — second-granularity timestamps collided in a fleet loop, and `date %N` is GNU-only so sub-second precision would be worse on macOS.
  **Consumer note**: exit `0` attests to *sandy's* goal (the sandbox is provisioned), not necessarily yours — verify the artifact you care about. Against a container started before handoff creation became unconditional (1.7.0), `--provision` correctly exits 0 and no handoff tree appears; since 1.10.0 the tree is mounted by default, so a consumer verifying it checks the container's mounts.
...
**Proxy source: local `COPY` from a checkout, `git clone` only for installed copies.** `generate_dockerfile_proxy()` builds the proxy from the operator's own `proxy/` source (`COPY proxy /src/proxy`, staged into a clean context) whenever sandy runs from a checkout that carries it — detected via `_sandy_proxy_local_src` on `dirname "$SANDY_SELF"`. It falls back to `git clone …/sandy && git checkout <ref>` only for installed single-file copies (no local `proxy/`), and an explicit `SANDY_PROXY_REF` or CI's `GITHUB_HEAD_REF` still forces the clone path. Two reasons: **correctness** — the clone checks out the current branch name, which fails outright on any *unpushed* feature branch (the normal state before merge), so a local `-dev` branch could not build the proxy at all; and **supply chain** — cloning built the security chokepoint from a *mutable remote ref* over the network, while `COPY` builds exactly the audited local source with no code fetched at build time. A `# proxy-src: <sha>` line in the generated Dockerfile carries a content hash of the local source so the existing "rebuild when the Dockerfile hash moves" trigger fires on a source change. Guarded by §49 (the `proxy (local): …` checks).
...
**A protected path that is a symlink goes through the symlink approval, wherever it points (R5, 1.13.3).** The three mount loops existence-gate with `-e` (files), `-f` (git files) and `-d` (dirs) — **every one of which follows a symlink** — and Docker dereferences a symlinked `-v` source. That last fact was the security review's one open question and is now **measured** on macOS Docker Desktop: `ln -s /etc /tmp/x && docker run --rm -v /tmp/x:/m:ro alpine ls /m` lists `/etc`. So a repository committing `.npmrc` as a link to a host file had that file mounted into the container.

`_sandy_resolve_symlinks` would have caught it — but only for targets under `$HOME`; it `continue`s on anything else, so a link to `/etc`, `/Volumes` or `/private/var` was mounted with **no prompt and no warning**. A link at a protected path is now surfaced for approval regardless of where it points. Note what was *not* chosen: **skipping the mount is the worst option**, because an unmounted protected path is writable and the agent then writes *through* the link to the host target. The entry is added to `DANGEROUS_SYMLINKS` only — deliberately not to the mount arrays, since the protected-path loop already mounts it `:ro` and the symlink mounts carry no `:ro`; the two mount arrays are consumed by index against each *other*, never against the approval list. An ordinary (non-protected) path pointing outside `$HOME` is still ignored, so this adds no new prompts. Guarded by §130.
...
Passive-safe to turn **on** from a repo (a repo may declare itself suspicious); `=0` from a workspace is approval-gated. **Profile mode is covered too, since 1.12.0 (#252).** `SANDY_CLAUDE_AUTH=profile` clears `CRED_JSON` before the #130 block runs, so every branch there was a no-op for it and the profile's own `credentials/<name>.json` rode in **unmodified, with its `refresh_token`** — a knob documented to shrink blast radius doing nothing for one mode, which is worse than not having it. The ephemeral copy is now stripped in that same block (never the host profile), `cred_mode` becomes **`profile-access-only`**, and a strip that cannot be performed or verified mounts **no profile at all**. **The trap worth knowing if you touch this**: the two file shapes spell the key differently — `.credentials.json` uses camelCase `claudeAiOauth.refreshToken`, a Console profile uses snake_case `refresh_token` (the WIF layout). `_sandy_strip_refresh_token` deleted *and verified* only the camelCase spelling, so reusing it unchanged would have deleted nothing and **returned success** — the same bug wearing a fix's clothes. It now handles both spellings in both halves, and §126(1) is the check that fails if either is dropped.
...
**Neither write follows a symlink (R2, 1.13.2).** The writer read its target with `JSON.parse`/`jq` and `mv -f`'d the result back, and neither step cared that a **repository** had committed the path as a link. Three measured consequences, all reachable by `gh pr checkout N && sandy`, host-side, before `docker run`, with no agent and no approval prompt: `.claude/settings.local.json` → `~/.claude/.credentials.json` made sandy **copy the host refresh token into the workspace** (any JSON object works — `~/.claude.json`, `~/.docker/config.json`, an ADC file); `.claude` committed as a **directory** link made the write land wherever it pointed; and because `mv -f` replaces a link with a regular file, `_sandy_resolve_symlinks` — which on the bare-`sandy` path does not run until ~1800 lines later — **found nothing left to approve**. Sandy's own write consumed the evidence its approval gate keys on.

The rule is now a property of the path, not a containment test: **no component below the anchor (`$WORK_DIR`, or `$SANDBOX_DIR` for the other target) may be a symlink**, walked with bash builtins. A symlink-free chain from the anchor *is* proof the file is inside it, so nothing needs canonicalizing — and `realpath` is GNU-only anyway. Sandy **refuses and warns** rather than resolving or replacing: refusing is what hands the case back to the symlink approval, where an escaping link belongs. Note that "canonicalize, then write" also stops the exfiltration and the escape yet is still wrong for exactly that reason — §129(4) is the check that fails on it. `mkdir -p` cannot be the detector: it succeeds silently on a symlink-to-directory. Guarded by §129.
...
**The delivery path is re-measured per Claude Code bump, and the sender frame is a fixture that can go stale.** Re-derived against 2.1.263 (§6a): `accept` logs `[uds-messaging] Routed user message to queue`, `hold` logs `[cross-session-inbound] held … cause=explicit-setting`, `refuse` logs `refused inbound peer message`. Two traps for anyone touching `test/acceptance-uds-delivery.sh`: the **`accept` path emits no `[cross-session-inbound]` line at all**, so absence of one is the success signature and must never be read as non-delivery; and the session key file is JSON — a relay authenticates with its **`peerToken`** field (⇒ class `peer`, which `crossSessionInbound` governs), *not* the file's contents and not `CLAUDE_CODE_MESSAGING_TOKEN`, which is the `childToken` and **bypasses the setting**. Auth is only enforced on Windows, so a wrong token is silently ignored elsewhere rather than failing loudly.

### The relay (a feature manifest `entry`)

**The handoff tree is gone as of 2.2.0.** `#352` removed the `inbox`, `outbox` and `peer` lanes, `#353` moved relay state to `$SANDBOX_DIR/relay-state` (mounted rw at `/opt/sandy/relay-state`), and `#355` removed what was left: the directory, `SANDY_HANDOFF_DIRS`, the `.handoff-enabled` marker, and the `handoff{}`/`handoff_enabled` reporting. A feature manifest names its own directories via `mounts`.

**Installing a relay is a manifest `entry`.** The `relay-bin/` slot and `SANDY_HANDOFF_RELAY`-as-a-config-key were removed in `#354`; both are hard errors naming the replacement. `SANDY_HANDOFF_RELAY` survives as the **internal channel** the entry travels through — deleting the variable deletes the mechanism.

`SANDY_RELAY` is the capability toggle, re-scoped in `#354` to gate the entry. `=0` stops it, **loudly** — the launch names the entry it declined to run, and `relay.disabled_by` records who set it.
...
**Sandy never parses JSON itself.** `node` or `jq` projects the manifest to flat TAB records and bash reads those. **Strict parse or refuse**: a launch that must read a manifest and finds neither refuses, and a manifest that cannot be read *in full* mounts nothing. This deliberately does **not** reuse `json_merge`, whose `command -v node &>/dev/null || return 0` silently does nothing — that shape must never reach a mount decision. The two projectors are held byte-identical by §135(20), because the jq one silently dropped the unknown-key check on its first draft: on a jq-only host a typo'd `mounts` would have been ignored rather than refused.

**`agent_args` (2.1.0, #348) lets a manifest contribute launch flags**, keyed by agent, passed at the launch that selects the sandbox — stateless (nothing written into the sandbox, nothing to reap) and read-only by construction (every file named lives on the `:ro` payload). Named `agent_args` rather than `args` because the manifest already has `entry` and `args` beside it reads as *the entry's* arguments. Feature args are **additive, ordered first, never suppressible**; several features compose in sorted slug order; only `-p`/`--print`/`--prompt` are filtered, with the warning naming the feature. Sandy defines the ORDER — precedence belongs to the agent's parser **where that parser has a precedence to apply**, which is not every flag (see below). A token containing whitespace is **refused, never split**, and like any refusal it takes the whole manifest down, so `--print-schema` now publishes `manifest.top_level_keys` for a consumer to gate on by MEMBERSHIP (a stripped `X.Y.Z-dev` compares equal to `X.Y.Z`, so a version gate is worthless here). What was applied is recorded per feature in the session marker and surfaced by `--print-state`, with three non-collapsible states: absent = a sandy that cannot answer, `{}` = none applied, populated = what and by whom. Guarded by §148.

**`agent_args` composes in the argv sandy BUILDS; whether it composes in EFFECT is the agent's parser's business, and for some flags it does not (2.3.0, #363).** A flag the parser reads **once** silently discards every occurrence but the last. That shipped: two features each contributed `--append-system-prompt-file`, sandy passed both in feature order, claude kept the second, and the first feature's prompt never reached the system prompt. Nothing caught it, because **every observable read healthy** — all mounts present and `:ro`, and the marker recorded *both* features, since the marker records what was **passed**. 2.1.0's own rationale is where the hole was: it justified "precedence belongs to the parser" with `--mcp-config` being variadic, generalizing one repeatable flag into an assumption about all of them.

Measured on Claude Code 2.1.278 (`FLAG exists FLAG /nope` errors on `/nope`; the reverse order does not): `--append-system-prompt-file`, `--append-subagent-system-prompt-file` and `--system-prompt-file` are **last-wins**; `--mcp-config`, `--add-dir`, `--plugin-dir` and `--plugin-dir-no-mcp` genuinely accumulate (explicit collectors in the bundle). **Re-measure on every Claude Code bump** — same discipline `acceptance-uds-delivery.sh` carries for the delivery path.

So sandy keeps a small published policy table (`--print-schema` → `manifest.agent_args_compose`), and **only for a flag listed there**:

- **`concat`** — the flag names a text file, so at **two or more** contributors sandy merges them in the order it would have passed them into `$SANDBOX_DIR/agent-args-composed/<agent>.<flag>.md`, mounts that `:ro` at `/opt/sandy/agent-args/`, and passes the flag **once**. Each section is headed with the contributing feature.
- **`report`** — the flag *replaces* rather than appends, so there is no correct merge: sandy names the collision and the loser and changes nothing.
- **not listed** — sandy assumes the agent accumulates and does nothing, which is every other flag of every agent.

**Nothing happens at one contributor**, and that is the whole safety argument: the argv is byte-identical to 2.2.0 and the feature's own live mount is what the agent reads. A value sandy cannot resolve to a host file through its own mount table (an operator `agent-args.<agent>` may name anything) is **reported but not merged** — fail open on what sandy cannot see, while still proving the collision. The composed file lives under `/opt/sandy`, not inside any feature root, for #353's reason: it is sandy's derivation from several features and belongs to none of them.

**`agent_args_composed` in the marker and `--print-state` is the half that makes this visible**, and it is a different question from `agent_args`: that one records what was **passed**, this one whether it can have taken **effect**. Entries carry `composed: true|false` so "merged" and "collision reported, argv unchanged" never collapse. Additive, so `schema_version` does not move. Guarded by §153, every check mutation-tested.

**And the launch now SAYS which features applied**, without `SANDY_VERBOSE`. A selected feature used to print nothing and a skip was verbose-only, so "installed" and "applied to this sandbox" were indistinguishable at launch — which is half of why the dropped prompt above took a debugging round to even confirm as applied. One line names each applied feature and what it contributed (mount count, which agents got args, whether it supplied an `entry`); a second names anything not applied **and why**. A `skip` for a feature that *was* selected means a declared mount whose source is missing, which is a real misconfiguration and warns; the `summary` record per selected feature is what tells those two apart.

**The manifest declares a `name`, never a destination.** Sandy computes it — `payload` → `/opt/sandy/features/<feature>`, `.` → `<home>/.<feature>`, anything else → `<home>/.<feature>/<name>`. There is no `to` field, so there is nothing to police; an earlier draft had one and would have permitted a mount over `/etc/sandy-session.json`. `name` and every `from` segment go through `[A-Za-z0-9._-]+`, no leading dot, no `..` — a computed root is only a root if its components cannot escape it. `mode` defaults to `ro`. One reserved top-level key, `feature`, is never interpreted; every other unknown key is **refused**, because a typo'd `mounts` that mounts nothing silently is the failure class this project spent three releases removing.

**`selected.json`** beside each manifest records which sandboxes were selected and why the others were not, for a consumer that executes nothing and so cannot run `--print-state`. It is per-slug files rendered with `printf`, not a read-modify-write, so `--remove-sandbox` can drop an entry with no parser on the host. **The note ships inside the file**: selection depends on the agents a launch resolved to, so for a sandbox that is not running the agent dimension is **unknowable**, not merely stale.
...
**Relay state moved out of `handoff/` in 2.2.0 (#353).** `.state` and `supervisor.log` now live at `$SANDBOX_DIR/relay-state`, mounted **rw** at `/opt/sandy/relay-state`. Producer-agnostic by design: an earlier draft proposed the feature's own instance tree, which `relay.source` disproved (two of the three producers had no feature) — and the reason that outlives even that is simpler, and the consumer's own: **these are sandy's supervisor's files, not the relay's**, so putting them in a feature's tree makes sandy's bookkeeping a tenant of the thing it supervises. Under `/opt/sandy`, not the agent home, because `~/.sandy` in-container would read as the **host** config root.

**The container path has exactly one source**: the host sets `SANDY_RELAY_STATE` beside the mount and the supervisor reads it. A relay locates its state by reading that variable, never by constructing a path — *"consumer constructs a path sandy owns"* has now been the fault line three times (#248 moved the home, #345 derived `relay.path` from the slot, #353 moved this). `--print-state` therefore also **reports** the directory as `relay.state_dir`, a **host** path — note `relay.path` in the same object is a **container** path, and the two frames must not be conflated. A one-time `mv` migrates an existing sandbox and runs **before** the `handoff/relay` mkdir; the other order moves the empty directory `mkdir` just created, on every launch.
...
Two build phases sit between the agent image and the per-project image: **2.5a** (`sandy-skills-base-{pack}`) installs heavy, rarely-changing deps (Playwright, Chromium) and rebuilds when its Dockerfile changes **or its parent image moves**; **2.5b** (`sandy-skills-{pack}`) fetches the pinned source and builds, rebuilding on a new version but fast because Chromium is already cached. At startup `user-setup.sh` symlinks `/opt/skills/{pack}/` into `~/.claude/skills/` and adds pack `bin/` dirs to PATH.
...
**Each skills cache key folds in its parent image id (2.1.0, #294).** Both images are built `FROM` an image sandy also builds, but each used to rebuild only when its *own* Dockerfile changed — so once `sandy-claude-code` moved, a skill-pack sandbox kept launching with whatever `user-setup.sh` and `entrypoint.sh` were baked in at its last build. Measured in the field: a gstack base from 2026-07-20 carried a `user-setup.sh` predating the relay supervisor (1.10.0), so every launch mounted the relay slot, exported `SANDY_HANDOFF_RELAY`, started the agent — **and never started the supervisor**, because the script that would start it was not in the image. `--print-state` then reported `relay.state="failed"` for a sandbox that had done nothing wrong. `BUILD_HASH` for the agent images already hashes those two files for exactly this reason; the skills chain inherits them through `FROM` and so never saw them change. `_sandy_parent_image_id` reads the `FROM` **out of the generated Dockerfile** rather than reconstructing the name — `Dockerfile.skills` is `FROM` the base image when a pack needs one and `FROM sandy-claude-code` when none does. A parent docker cannot resolve hashes as `absent`, never as an empty string: empty would hash identically to a missing parent and silently stop invalidating. Guarded by §150.
...
**Workspace mount path**: mounted at a path mirroring the host's `$HOME`-relative location — `~/dev/sandy` → `/home/sandy/dev/sandy`. Outside `$HOME` it falls back to the real host path. Passed as `SANDY_WORKSPACE`.
...
**The `gitdir:` line is repository content, and it is validated (R5, 1.13.3).** It used to be resolved with a bare `cd` and no containment check, then bind-mounted with **no `:ro`**. Measured: `gitdir: ../../.ssh` produced `-v $HOME/.ssh:/home/sandy/.ssh` **read-write**, and `gitdir: /etc` produced `-v /etc:/etc` **read-write**. Write access to `~/.ssh` is host code execution — an `~/.ssh/config` `ProxyCommand` fires on the user's next `git push`. The symlink approval cannot see any of it: `.git` is a regular **file**, not a link, so the scan finds nothing.

The gate is **structural, not a path prefix**, and that choice is load-bearing: a legitimate submodule gitdir is deliberately *outside* `$WORK_DIR` (it lives under the superproject) and a `--separate-git-dir` may be anywhere the user chose, so any "must be under X" rule rejects real setups. *Is it actually a git directory* — `HEAD` plus `objects/` plus `refs/` — is the property that separates those from `~/.ssh`, `~/.aws` and `/etc`, and it needs no canonicalization. A directory an attacker could already furnish with that layout is one they can already write, so the check gives nothing away. A failure is a hard error with the `.fatal` marker, so `--start` fails in ~1s rather than polling out its timeout.

**The gitdir is mounted rw, so its own `config`, `hooks/` and `info/` are now overlaid `:ro`** — git must write the gitdir (index, refs, objects), but the host-code-execution surfaces inside it need the same protection every other repo gets. Nothing covered them before: `_protect_submodule_gitdirs` walks `$GITDIR_HOST/**modules**` (gitdirs *nested* under this one) and never `$GITDIR_HOST` itself, and `_sandy_protected_git_files` targets `$WORK_DIR/.git/config`, which does not exist when `.git` is a file. Measured on a real submodule worktree: the gitdir produced exactly **one** mount flag, read-write, with no `:ro` over `config` or `hooks/`. So a wholly trustworthy repo still handed the agent host-write access to that submodule's hooks. Guarded by §130.

**Screenshots / `/ss`**: `SANDY_SCREENSHOT_DIR=<host-path>` (privileged) mounts a host screenshot folder `:ro` at `/home/sandy/screenshots`, exposed as `$SANDY_SCREENSHOTS_PATH`, and generates a per-agent `/ss` surface at sandbox setup (claude: `~/.claude/commands/ss.md`; gemini: `~/.gemini/commands/ss.toml`; codex: a description-matched skill; opencode: manual). All are powered by `/usr/local/bin/sandy-ss-paths`, callable from any agent's bash escape. Validation rejects shell metacharacters and overly-broad targets (literal `$HOME`, `/`); a non-existent directory is warn-and-skip, so Docker never auto-creates an empty stub on the host. No default — unset disables the feature entirely.
