...
Set `SANDY_SCREENSHOT_DIR=<host-path>` (passive-safe, set in `~/.sandy/config` or per-workspace `.sandy/config`) to mount a host folder of screenshots into the container at `/home/claude/screenshots` (read-only). Sandy exposes the in-container path as `$SANDY_SCREENSHOTS_PATH`. When set, sandy generates a per-agent `/ss` skill at sandbox setup so the agent can "see" what the user just captured.
...
- `0` — **off** (default). Legacy behavior: Linux uses iptables RFC1918 DROPs; macOS has no network isolation (warning banner above).
- `1` — **permissive**. The agent routes through a proxy sidecar that blocks only private/LAN/link-local/CGNAT/cloud-metadata destinations and allows all internet. Closes F2 with ~zero tool friction (any public host an agent needs just works). This is the **intended default-on posture for 1.0** — it gives Linux-parity LAN isolation on macOS without an allowlist to maintain.
...
**Topology.** Two Docker networks are created per session: an `--internal` **sidecar** bridge that the agent joins (no route off the bridge — this is the isolation) and a normal **egress** bridge that only the dual-homed `sandy-proxy` container joins (its internet leg). The proxy is the sole path off the sidecar, so it is also the single policy chokepoint. The agent's resolver is pointed at the proxy (`--dns <proxy-ip>`); the proxy's DNS responder redirects permitted names to its own sidecar IP so traffic funnels through its listeners. The proxy gets a fixed IP (sandy picks the first non-overlapping `/24` from a small candidate list so `--ip` is usable). iptables is **not** applied in proxy mode (the topology is the isolation, and RFC1918 DROPs would break the proxy's own `host.docker.internal` forward).
...
The replacement defense is **session-end detection**: sandy records which protected dirs existed at launch in `$SANDBOX_DIR/.protected-existed-at-launch`, and on session exit walks the same paths looking for new appearances. Anything with content that didn't exist at launch is reported as a yellow warning with a `rm -rf` remediation. This catches the case where an agent (via prompt injection, skill bug, or just a bad task) writes `.git/hooks/post-checkout` or `.github/workflows/ci.yml` — the user sees the warning before they next run `git pull` / `git push` and can decide whether the write was legitimate or needs to be reverted.
