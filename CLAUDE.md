...
**The container user is `claude` and its home is `/home/claude` — agent-specific naming in a five-agent sandbox.** Renaming both to `sandy` is tracked as **#248** and is a **2.0.0** change, not a minor: `/home/claude` is baked into persistent sandbox state (venv shebangs, `GOPATH`, `PYTHONUSERBASE`, npm/cargo metadata), so it would force `SANDY_SANDBOX_MIN_COMPAT` above the `1.0.0` ceiling the forward-compat promise imposes, and it moves published consumer contracts (`~/.handoff/*`, `/ss`, `SANDY_WORKSPACE`).

**`sandy --exec` — a shell in the running container, and why the hand-rolled form is wrong.** `docker exec -u claude` runs as **uid 1001**, not the host uid: the image does `useradd -u 1001 claude`, sandy bind-mounts a generated `/etc/passwd` carrying the host uid, and docker resolves `-u <name>` against the **image** filesystem rather than the runtime mount. Confirmed on a real container (`docker exec -u claude <c> id` → `uid=1001`). The visible tell is a prompt reading `I have no name!`; the damaging part is writes landing as the wrong owner on a workspace mount owned by the host user. `--exec` therefore uses numeric `-u $(id -u):$(id -g)`, sets `-w` to the container-side workspace path, and passes `HOME` explicitly — docker only derives HOME when it can resolve the user, and as root `HOME=/root` is on the read-only rootfs, which is why an in-container `codex logout` returned `Read-only file system (os error 30)`. Resolution is daemon-label first, then a **fully anchored** foreground name match (a workspace named `proxy` yields agent `sandy-proxy-<hash>` beside sidecar `sandy-proxy-proxy-<hash>`). Exit `4` = no running container; otherwise the command's own status. `--dry-run` prints the command and runs nothing. Guarded by §120, which asserts the **real** argv as well as the `--dry-run` rendering — they are separate render paths and a first cut of the guards missed a mutation that changed only one.
...
**One predicate, two callers** (the `_sandy_handoff_classify` rule): `_sandy_features_scan` is what `--print-state` reports *and* what the launch mounts from, so a reporter that sees a feature the mounter does not is impossible by construction. It tests **`-L` before `-d`/`-f`** — `-d` follows links, so a symlink to a directory would pass every other test — and it globs `*` **plus** `.[!.]*` **plus** `..?*`, because a bare `*` skips dotfiles and an entry sandy will not use *and does not mention* is the silent skip this design exists to retire. **A valid entry is a regular file OR a directory** (contents, size, mode and owner are never examined), named `[A-Za-z0-9._-]+` with no leading dot and no `..` substring, and it must be a **direct child** — sandy never descends. That full rule is published in `SPEC_INTROSPECTION.md`, not just implemented here, **because consumers write these markers**: the first one to do so mirrored the predicate as "a regular file" and would have refused directories sandy honours, producing a sandbox sandy called enrolled and the consumer did not. Guarded by §134(20). `--reset-sandbox` **preserves `features/`** (without it, a reset silently un-enrols the sandbox); `--remove-sandbox` names each entry in the plan.
...
- **`_sandy_handoff_classify()`** is **one predicate with two callers** — `--print-state` reports it and `--provision --all` acts on it. A reporter that says `ok` while the provisioner still provisions (or the reverse) is invisible drift; same rule as `_sandy_reap_orphan_networks` serving both `--gc` and `--prune-orphans`. It asserts only what sandy *guarantees*: sandy creates these with a bare `mkdir -p` and sets **no mode and no owner**, so inventing a mode standard here would be reporting a rule nothing enforces. It checks existence, that each is a real directory, that none is a **symlink** (`mkdir -p` never makes one, and `inbox`/`peer` are the host-written `:ro` halves, so a redirected one is an integrity problem), and that each is owned and usable by the invoking user. Bash builtins only (`-d -L -O -r -w -x`) — `stat` would need the GNU `-c` / BSD `-f` split for no more accuracy. **`-L` is tested before `-d` on purpose**: `-d` follows links, so a symlink to a directory would otherwise pass every other test.
- **`--print-state` → `handoff{state, dirs, problems}`**, both modes. `state` is `ok|missing|wrong`. It is **read-only by construction, and that is a requirement rather than an implementation detail** — the consumer relies on a hand-made pair staying distinguishable, so a field that quietly repaired would destroy the property it exists to report on. `ok` is a **host-side** statement: it does not mean the tree is mounted in any container, since `--print-state` reads no configs and inspects no container. `handoff_enabled` carries the same caveat and has been misread as a mount signal before.
- **`--provision --all`** provisions every **known** sandbox whose pair is missing or wrong, serially, composing the per-workspace `--provision` verbatim (the `--stop-all` rule: one lifecycle path, so the fleet form cannot drift from the single form). **Scope is a guarantee, not a limitation**: it enumerates `$SANDY_HOME/sandboxes/*/`, so a workspace that has never been launched has no directory and cannot be reached even in principle — the sandbox directory *is* the registry, and enrolling a new workspace stays a deliberate `--provision --workspace P`. It **re-classifies after each child** rather than trusting the exit code, because `--provision` attests to *sandy's* goal, not the caller's. A sandbox whose recorded workspace is gone cannot be provisioned: it is **named, counted unprepared, and the run exits non-zero** — never skipped into a false `0`, because an exit `0` hiding one unprepared sandbox fails invisibly, and an orphan is *clearable* (`--remove-sandbox --orphans`), so the refusal leads somewhere. Guarded by §124.

  **A sandbox with a LIVE session is skipped at the PLANNER, not at the child (1.14.1).** The planning loop originally filtered on the classification and on the workspace existing, and checked liveness not at all — a running sandbox with a bad pair entered the plan, the child no-opped at its own guard 1 or 2 and returned `0`, the re-classification then found the pair still bad *because nothing had run*, and it was filed as `<name> (provisioned, but the pair is still missing)`. It was **skipped, not provisioned**, and the run exited `1` saying otherwise. Nothing was ever torn down — the per-workspace guards were always intact — so this was a **reporting** defect, of the class where a correct mechanism asserts something untrue and an exit code conflates *failed* with *skipped because live*. The planner now applies the same two predicates the per-workspace form does (workspace mutex, then a running daemon container — one batched `docker ps` for the sweep, the `--stop-all` idiom), reusing the `_rms_lock_live`/`_rms_container_live` shapes rather than spelling the question a third way. **C1 is preserved, not relaxed**: a live sandbox with a bad pair leaves the goal state unmet and no work here can fix it, so it stays exit `1`, named and counted — it differs from an orphan only in being trivially clearable (stop the session, re-run). The lock test runs **before** the workspace-gone test on purpose: a running sandbox is a stronger fact than a missing `WORKSPACE.json`, and telling the operator to `--remove-sandbox --orphans` something that is running would be dangerous advice. Guarded by §133, whose live and dead-pid cases are the **same fixture with one byte changed** — the live case alone would pass against a planner that skipped for an unrelated reason.

  **Declining the confirm exits `0`**, matching the other seven members of the family (`--stop-all`, `--gc`, `--reset-sandbox`, `--remove-sandbox`, `--doctor --fix`, per-workspace `--provision`, `--update-sessions`). `--provision --all` exited `1` and was also the only one whose message was a bare `Aborted.` rather than `Aborted — nothing X.` — an outlier on both axes at once is an inconsistency, not a contract. §133(8) ratchets that no abort site in the family exits non-zero.

**`SANDY_HANDOFF_RELAY=<path>`** (privileged) is the mechanism that drains and fills those directories; sandy does not need to know what it does, only how it runs. It **forces `SANDY_HANDOFF_DIRS=1`** and mounts a third rw dir at `~/.handoff/relay` (state + `supervisor.log`).
...
- **A configured relay that cannot start FAILS THE LAUNCH (criterion 7)** — deliberately not the "degrade to a warning" posture, because the warning would leave the receive surface open (`accept`) with nothing delivering. Two detection points: **host-side**, sandy maps the path back to the host where it can (workspace-relative → `$WORK_DIR/<rel>`; absolute under `$SANDY_WORKSPACE/` → `$WORK_DIR/<rest>`) and exits 1 before `docker run`; **in-container**, for what the host cannot see (image-only path, unmounted `~/.handoff/relay`, no `flock`), it logs `ERROR:` and `exit 1`s so the container dies before any tmux session exists (`--start` reports crash-looping, exit 7). Each **host-side** refusal additionally drops the `.fatal` marker the `--start` readiness loop watches, so the client fails in ~1s with **exit 6** (refused before launch) rather than polling out its 600s timeout — the supervisor dies instantly and without the marker the client simply looks hung, which is what `acceptance-handoff-dirs.sh` E10 hit. The `flock` check precedes the backgrounded subshell — inside it, `exit 1` would kill only the subshell. The `~/.handoff` collision is the same hard error. Note the ordering: the host-side path check runs **before** the skip decision below, so a broken path errors even on a launch that would not have started the relay.
- **Three deliberate skips, not failures (criterion 8)**: headless (`-p`), **`sandy --remote`** (no tmux session to target, and a supervisor that never gives up would restart every 60s for the container's life), and **`--provision`** (1.12.1 — it starts a session only to prove it comes up and then stops it, so a relay would live about ten seconds and deliver nothing, while criterion 7 would *fail* the launch if it could not start — defeating the one command whose job is materialising the handoff pair). The `--provision` case was found in the field, not in review: a host-wide `SANDY_HANDOFF_RELAY` naming an in-container path that unprovisioned sandboxes do not have made **every** `--provision --all` target crash-loop (`--start` rc=7), so 44 of 52 sandboxes failed in a row for a reason unrelated to provisioning them. Skipping is also the *safer* resolution rather than a concession: with the key unset, `crossSessionInbound` resolves to `refuse` for that throwaway launch instead of `accept`, so it never opens a receive surface. The host **unsets the key** with `SANDY_HANDOFF_RELAY not started (<reason>); crossSessionInbound will default to refuse`, so the default resolves to `refuse` and the marker records `handoff_relay: false`; the `SANDY_HANDOFF_DIRS=1` implication is kept. The in-container call site is gated on both conditions as belt-and-suspenders.
- **Path validation** (host-side): absolute or workspace-relative; rejects whitespace, shell metacharacters, and any `..`/`.`/empty segment.
- **Env contract**: `SANDY_HANDOFF_INBOX`/`_OUTBOX`/`_PEER`/`_RELAY_STATE` (`_PEER` added in 1.10.0, additive). Honestly — they are exported in `user-setup.sh`'s own shell, so they also reach the tmux server and every pane, and the relay inherits the full container env including `CLAUDE_CODE_OAUTH_TOKEN` if present.
