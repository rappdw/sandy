...
# ---- banner ----
printf "%ssandy doctor%s  —  host readiness check\n" "$BOLD" "$RESET"
...
case ":$PATH:" in
    ...
    *)
        ...
        # shellcheck disable=SC2088  # display strings, not paths we expand
        case "$_shell" in
            zsh)  _rc="~/.zshrc" ;;
...
if [ -f "$HOME/.claude/.credentials.json" ]; then
    # shellcheck disable=SC2088  # a message telling the user where to look
