...
Identity is proven via a new env-gated marker: with `SANDY_TEST_PANE_TAGS=1`, sandy's multi-agent branch sets a **`@sandy_pane_agent` tmux pane option** (plus a `[sandy:pane-agent] <agent>` stdout line for logs) ahead of each pane's real agent command; the harness reads identity from that pane option via `list-panes -F '#{@sandy_pane_agent}'`. The pane option is the robust source — it binds to the pane running each agent's command and, unlike a scrollback marker (which a *real credentialed* agent wipes when it redraws its small 2×2 pane) or `select-pane -T` (OSC-2-clobberable), the agent cannot touch it. A per-combo retry + settle gate further absorb transient container-startup races. `SANDY_TEST_PANE_TAGS` is an internal env-only test hook (like `SANDY_AUTO_APPROVE_PRIVILEGED`) — not in any config-key tier, no `_sandy_key_metadata` row, never settable from a committed `.sandy/config`. Guarded structurally by `run-tests.sh §80`.
