...
Sandy runs Claude Code, Gemini CLI, OpenAI Codex CLI, or any combination of them side-by-side in a Docker container with agent permission checks disabled — so the agent works without interruption while your system stays protected:
...
|---|---|---|
| `SANDY_AGENT` | `claude` | AI agent(s) to run. Single: `claude`, `gemini`, `codex`. Multi (comma-separated, 2 or 3 panes in tmux): e.g. `claude,gemini` or `claude,gemini,codex`. Alias: `all` = `claude,gemini,codex` |
...
Sandy runs any combination of Claude, Gemini, and Codex side-by-side in a single tmux session, selected via comma-separated values in `SANDY_AGENT`:

```bash
SANDY_AGENT=claude,gemini              # two panes
SANDY_AGENT=claude,codex               # two panes
SANDY_AGENT=claude,gemini,codex        # three panes
SANDY_AGENT=all                        # alias for claude,gemini,codex
```

Panes appear in the order listed. Each agent has its own config at `~/.claude`, `~/.gemini`, and `~/.codex` respectively. All panes share the same workspace mount. Exiting one pane leaves the others running. Single-agent modes use their own Docker images (`sandy-claude-code`, `sandy-gemini-cli`, `sandy-codex`); any multi-agent combo uses the `sandy-full` image, which bundles all three CLIs.

**Feature support in multi-agent mode**: skill packs apply to the Claude pane only. Telegram channels use the host-side relay and are routed to pane 0 by default — override with `SANDY_CHANNEL_TARGET_PANE=0|1|2`. `--remote` is not supported in any multi-agent combo.
