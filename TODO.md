...
Sandy currently seeds two plugin marketplaces via `extraKnownMarketplaces` in settings.json: `claude-plugins-official` (Anthropic) and `sandy-plugins` (rappdw). Consider seeding additional community marketplaces to give users a richer plugin catalog out of the box.
