...
> literal boolean keys, regardless of what `SANDY_AGENT` the sandbox was last
> launched with — this fast-path dispatch cannot read a workspace's
> `SANDY_AGENT`, and the fixed key set means a consumer never sees a varying
> shape. Emitted identically in **both** `--print-state` and `--print-state
> light` (five `[ -f ]` builtin tests, zero extra process spawns — the
> `workspace_exists` precedent, not the `size_bytes` one). Each value is
> `true` iff `$SANDBOX_DIR/agent-args.<agent>` exists on disk — **file
...
- **Feature-gating guidance: a stripped version is NOT evidence that a feature exists.** This is the same regex as the cache-keying bullet above with a worse consequence, and it is stated separately because reading that bullet does not prevent it. `X.Y.Z-dev` and `X.Y.Z-rcN` compare **equal** to `X.Y.Z` under any `(\d+)\.(\d+)\.(\d+)` match — and under sandy's own `_ver_lt`, which opens `local a="${1%%-*}" b="${2%%-*}"`. So a consumer gating "refuse below `X.Y.Z`" is satisfied by **every build on the `X.Y.Z-dev` line**, including every commit that predates the feature being gated on. The failure is not a stale cache; it is proceeding against a sandy that cannot do the thing, with no error.

  **Probe the capability, not the number**, wherever the capability is observable — and for this surface it always is. A config key is in `--print-schema`'s `config.privileged_keys` / `config.passive_keys` iff sandy supports it; a `sandboxes[]` field is emitted by `--print-state` iff sandy emits it. Both are one JSON read on a fast path that needs no Docker and no workspace, and both test the thing the consumer actually depends on. Keep a version floor if you want one, but do not let it stand in for a capability check.

- **Pre-1.7.0 forwarding hazard.** `--print-version` did not exist before 1.7.0, and sandy's main parser forwards any flag it does not recognize straight through to the wrapped agent instead of erroring — so calling `--print-version` against an older sandy does not fail fast, it silently attempts a full container launch. Probe with `--version` first: its stdout is **exactly** `sandy <full_version>`, byte-for-byte, guaranteed on every sandy old and new and pinned by `run-tests.sh` §93(2)/§93(5).

  **Match the full string; do not parse a numeric triple out of it.** Doing so walks straight into the bullet above: on a `1.7.0-dev` build that predates `--print-version`, a stripped version reads `1.7.0`, a `>= 1.7.0` gate passes, and the call then forwards to the agent and attempts the container launch this bullet exists to prevent. Treat any `-dev`/`-rcN` suffix as **not** the released version. That is conservative in the right direction: a false negative costs one redundant `--version` call, a false positive costs an unintended container launch.
...
- **`1.15.0`**: two new `sandboxes[]` fields, `features` (a JSON array of marker names, sorted) and `feature_problems` (a JSON array of `"<entry>: <reason>"` strings), both emitted in BOTH modes; one new field in the session marker, `sandbox_name` (#303); and one new privileged config key, `SANDY_FEATURES_DIR` (#305). Additive, so `schema_version` stays `1`.
  ...
  **The marker rule, in full — because consumers WRITE these markers.** An entry is valid iff it is **not a symlink** (tested before the type test, since `-d`/`-f` follow links), its name matches `[A-Za-z0-9._-]+` with **no leading dot** and **no `..` substring anywhere**, and it is **either a regular file or a directory**. Nothing else is examined: not size, not mode, not owner, not contents.
  ...
  | valid | rejected, and named in `feature_problems` |
  |---|---|
  | regular file (including zero-byte) | symlink to a file, symlink to a directory |
  | **directory** (contents ignored — stash notes there if you like) | fifo, socket, device |
  | any mode, including `000` | leading dot (`.hidden`), `..` prefix, `..` anywhere in the name |
  | names like `UPPER_ok-1.2` | any character outside `[A-Za-z0-9._-]`, e.g. a space |

  **Direct children only** — sandy never descends into a marker directory, so `features/x/y` is invisible. A consumer mirroring this predicate should mirror it exactly: refusing a shape sandy honours (a **directory** is the one people get wrong) produces a sandbox sandy considers enrolled and the consumer does not, which is the silent disagreement `feature_problems` exists to prevent. If you deliberately accept less, say so in your own output rather than refusing quietly.
...
- **`2.1.0` (#345, #344)**: two new `relay` sub-fields, `source` and `executable_present`, plus a **fix** to `relay.path`. Additive, so `schema_version` stays `2`.
  ...
  **`relay.path` was wrong in this surface and is now fixed.** It was derived from the slot directory alone, so a manifest entry and an explicit path both reported `null` — measured as byte-identical objects. It now comes from the launch marker, so it names whatever actually ran.
...
- **`2.2.0` (#354)**: `relay.slot` can no longer report `present` — the `relay-bin/` slot is removed, so its reachable values are `absent` and `disabled`. `relay.source` loses `explicit` and `slot` for the same reason: a manifest `entry` is the only producer. **Both fields keep emitting**, and `--print-state` still reads the retired values back out of markers written by an older sandy, because it reports the LAST LAUNCH. `schema_version` is unchanged — no field was removed; the removals are bundled into the single move in #355.

  `SANDY_RELAY` is **re-scoped, not retired**: it now gates a manifest `entry`, which it did not before. A launch that declines to start a declared entry says so by name and records `disabled_by`.
