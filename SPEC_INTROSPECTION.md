...
As of 1.7.0, sandy guarantees a stronger, test-pinned contract for these three flags (#160): **exactly one JSON document on stdout and nothing else** — including in JSON-shaped failure cases. Concretely:

- `--print-schema` and `--print-state` (both modes) always write exactly one JSON document to stdout and zero bytes to stderr, on every exercised code path.
...
10. **§92 (1.7.0, #160) stream contract pin** — for `--print-schema`, `--print-state` (both modes), and `--validate-config` (valid/unknown/privileged-from-passive fixture, missing-file, and no-argument cases), asserts stdout is exactly one JSON document (or exactly 0 bytes in the no-argument case) and stderr is exactly 0 bytes (or non-empty and contains `ERROR` in the no-argument case) — two-sided, so a future `info`/`warn` call landing on stdout is caught even though it would leave stderr untouched
