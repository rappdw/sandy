...
| `SANDY_HANDOFF_RELAY` | privileged | unset | 1.10.0 | experimental | Privileged. Path of an executable INSIDE the container (absolute, or relative to the workspace, e.g. .sandy/relay.sh) that sandy runs as a container-level process: a sibling of the tmux server, not a pane and not a child of any agent session. Started once per container by user-setup.sh before the tmux session exists (never in headless -p runs, never under --remote (no tmux session to target), and never under --provision (which starts a session only to verify it and then stops it, so a relay would live ~10s and criterion 7 would fail the very launch that materialises the handoff pair) — in both the key is dropped host-side, so crossSessionInbound defaults to refuse and the marker records handoff_relay=false), singleton via flock, restarted on death with exponential backoff (1s doubling to 60s, reset after a 60s+ run), never overlapping, never given up on. Setting it forces SANDY_HANDOFF_DIRS=1 (over an explicit opt-out — the relay cannot run without its mounts; the dirs are on by default since 1.10.0 anyway) so $SANDBOX_DIR/handoff/relay is mounted rw at ~/.handoff/relay (relay state + supervisor.log). Env contract: SANDY_HANDOFF_INBOX, SANDY_HANDOFF_OUTBOX, SANDY_HANDOFF_PEER, SANDY_HANDOFF_RELAY_STATE, SANDY_AGENT, SANDY_WORKSPACE; enumerate live sessions with /usr/local/bin/sandy-handoff-sessions. A configured relay that cannot start FAILS THE LAUNCH, never warn-and-proceed: host-side (exit 1 before docker run) for whitespace, shell metacharacters, .. segments, a ~/.handoff collision, and any path the host can see (workspace-relative, or absolute under the workspace mount) that is missing or not executable; in-container (user-setup.sh exits 1, the container dies, --start reports crash-looping/exit 7) for an image-only absolute path that is missing or not executable, a missing ~/.handoff/relay mount, or no flock. So handoff_relay=true in the marker means the relay was started or the session did not come up. In daemon mode the relay (and anything else planted at the uid) outlives sessions until the container is recreated — run sandy --update-sessions --yes on a 24h cron. Recorded in /etc/sandy-session.json as handoff_relay. |
| `SANDY_FEATURES_DIR` | privileged | unset | 1.15.0 | experimental | Privileged. Host directory whose children are SHARED feature payloads, mounted READ-ONLY at /opt/sandy/features/<name> into only those sandboxes carrying a matching $SANDBOX_DIR/features/<name> marker (#304). One install, one version, N sandboxes -- instead of a per-sandbox copy the consumer has to keep in sync and byte-compare. GATED ON THE MARKER, never unconditional: an unconditional shared mount would install into EVERY sandbox including ones running a different agent, which is the incident the `agents` field exists for (a host-side tool enrolled 51 sandboxes and installed a per-agent component into each; one ran codex, and the only symptom was a process exiting non-zero once a second into a log nobody was watching). The :ro MOUNT is the boundary, not permission bits: the container runs as the host uid, so a writable payload is one the agent could rewrite; under :ro the write returns EROFS. HONEST LIMIT, as for the relay slot: sandy guarantees the FIRST executable only -- a binary that execs out of a writable directory is replaceable at the second link. Mounted under /opt, not the agent home, because that tree is rw tmpfs and /home/claude moves in 2.0 (#248). Privileged for the same reason SANDY_SCREENSHOT_DIR is: it chooses a host path to mount, so a committed workspace config must not be able to set it. Shell metacharacters and overly-broad targets (literal $HOME, /) are hard errors; a MISSING directory is warn-and-disable, because Docker would otherwise auto-create an empty host dir at bind time. A marker naming a feature with no matching source WARNS rather than no-opping -- an unnamed misconfiguration is indistinguishable from a working one. |
...
| `SANDY_HANDOFF_DIRS` | passive | `1` | 1.7.0 | experimental | Create and mount the per-sandbox cross-workspace handoff directories (#132 substrate): $SANDBOX_DIR/handoff/outbox rw at ~/.handoff/outbox, handoff/inbox READ-ONLY at ~/.handoff/inbox, handoff/peer READ-ONLY at ~/.handoff/peer (a second host-written inbound directory, 1.10.0) and handoff/relay rw at ~/.handoff/relay (relay state + supervisor.log). DEFAULT 1 SINCE 1.10.0 (was 0): every sandbox gets the tree on every launch, idempotently, so a host-side process can be pointed at any sandbox without a per-sandbox enabling step that has to be remembered, provisioned and verified. Set 0 to opt a host or workspace out: nothing is mounted, no env vars are added, and ~/.handoff does not exist inside the container. Both values are passive-safe for the reason the key always was: a directory confers no reach on its own — nothing lands in inbox/peer unless something on the host writes there and nothing leaves outbox unless something on the host reads it — so 1 is what a workspace could always set without approval and 0 only tightens. The two things that DO confer reach are unchanged and privileged: SANDY_HANDOFF_RELAY (off unless set; setting it forces this key to 1 because the relay cannot run without its mounts) and the crossSessionInbound pin (accept only when a relay is set and will start). Residual: the outbox persists across sessions, so a repo can stage content before an operator ever approves a relay; sandy --reset-sandbox clears it. The host directories are created unconditionally regardless of this key (since 1.7.0) so directory PRESENCE carries no information; consumers verify the MOUNTS (inside: test -d ~/.handoff/inbox ~/.handoff/outbox ~/.handoff/peer; outside: docker inspect .Mounts), never the directories or a marker. The operator marker $SANDBOX_DIR/.handoff-enabled (sandbox TOP level, never mounted, survives --reset-sandbox) now means FORCE ON for this sandbox even when config says 0 — the per-sandbox operator override of an opt-out, privileged by location like agent-args.<agent>, and a no-op when nothing opts out. It lives at the top level (never under claude/, which is mounted and agent-writable) so the agent cannot create it. |
...
| `SANDY_RELAY` | passive | `1` | 1.11.0 | experimental | Relay CAPABILITY toggle (#258). Names no path and asserts no payload: it means 'run the installed relay if there is one'. The relay is installed HOST-SIDE by writing one executable entry named 'relay' into the per-sandbox slot $SANDBOX_DIR/relay-bin/, which sandy mounts READ-ONLY at /opt/sandy/relay -- so installing a connector is one file write, with no workspace config key, no privileged-key approval prompt and no generated wrapper. Exactly one entry: a relay.d-style set would break connectors whose claim lock refuses a second consumer on one notice directory. Default 1, safe because the capability is INERT without host-side state a repository cannot create -- 'no entry in the slot' is a legitimate resolved state, not a broken path (contrast a hypothetical default for SANDY_HANDOFF_RELAY, which names a file sandy does not install, so every unprovisioned sandbox would fail to launch). The read-only MOUNT is the boundary, not permission bits: the container process runs as the host uid and OWNS the entry, so bits bind nothing and the agent could chmod its own relay; under a :ro mount the write returns EROFS. An adapter can write files, it cannot create a mount. HONEST LIMIT: sandy guarantees the FIRST executable only -- a relay that execs a daemon out of a writable directory is replaceable at the second link. An entry that exists but is not executable is an ERROR that fails the launch, never silently absent. A present entry forces SANDY_HANDOFF_DIRS=1 (the relay needs ~/.handoff/relay for state) and makes crossSessionInbound default to accept, on the same condition it always did. Both values are passive-safe: 0 only tightens. But because a CLONED repo shipping 0 would silently disable an operator's fleet connector, the resolved state records disabled_by (env, host or workspace) in /etc/sandy-session.json and --print-state -- visible, not forbidden. Skipped, not failed, in headless (-p) and --remote runs, exactly like SANDY_HANDOFF_RELAY. Reported as relay.slot (absent, present or disabled) in the session marker -- launch INTENT, since the marker is written before docker run and the relay starts in-container -- and as live relay.state (absent, started, failed, looping or disabled) in --print-state. |
...
│   └── relay/                 # → /home/claude/.handoff/relay (rw); relay state + supervisor.log + .state/.startup; created unconditionally alongside outbox/inbox
├── relay-bin/                 # → /opt/sandy/relay (:RO) — the relay slot (1.11.0, #258); exactly one entry, `relay`;
│                              #   created unconditionally, mounted only when it supplies the relay that will run;
│                              #   preserved by --reset-sandbox (operator state, like .handoff-enabled)
...
| Field | Meaning |
|---|---|
| `schema` | Marker schema version (currently `1`; `effort`, `permission_mode`, `cross_session_inbound`, `handoff_relay`, `cred_mode`, and `sandbox_name` are all additive fields). |
...
```bash
-v "<PROFILE_TMPDIR>:/home/claude/.config/anthropic"     # rw, ephemeral; holds ONE profile:
...
-v "<SANDBOX>/handoff/peer:/home/claude/.handoff/peer:ro"
...
SANDY_HANDOFF_PEER=/home/claude/.handoff/peer      # 1.10.0, additive: the second (read-only) inbound directory
```
