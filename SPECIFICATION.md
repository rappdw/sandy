...
**Non-default git hooks (`core.hooksPath`).** `.git/hooks/` is protected, but a repo (or the user's global git config) can redirect hooks elsewhere via `core.hooksPath` (e.g. `core.hooksPath = .githooks`). `_sandy_extra_hooks_dir` resolves the effective value at launch (`git -C "$WORK_DIR" config --get core.hooksPath`), canonicalizes it (`pwd -P`, so a symlinked hooks dir or a `..` traversal can't slip the target past the check), and — when it points at a directory **inside the workspace** other than `.git/hooks` — mounts it `:ro` and records it in the launch snapshot, existence-gated exactly like the static protected dirs. A hooksPath resolving **outside** the workspace needs no protection (only the workspace is writable to the agent). This complements the `.git/config` `:ro` mount, which blocks *injecting* a new `core.hooksPath`; this handles a *pre-existing* one that points at an otherwise-unprotected directory (Pillar "Week of Sandbox Escapes" #4 variant). **Residual:** the create-fresh case (hooksPath set but its directory absent at launch) is existence-gated to no mount and is not yet in the session-end detector's static walk — a detection-only gap deferred to a generalized auto-execution-config sweep. Standing rule this reinforces: never gate a trust decision on a filename/path pattern without a symlink/indirection check (`pwd -P` here; the same caution applies to a symlinked `.git` or an external `GIT_DIR`).
