...
| `SANDY_HANDOFF_DIRS` | passive | `1` | 1.7.0 | deprecated | DEPRECATED since 2.0.0, and as of 2.2.0 it governs ONE directory. Create and mount $SANDBOX_DIR/handoff/relay rw at ~/.handoff/relay (relay state + supervisor.log). The inbox, outbox and peer lanes were REMOVED in 2.2.0 (#352): a feature manifest names its own directories via mounts, and every lane was measured empty across the fleet before removal. Set 0 to opt out: nothing is mounted and ~/.handoff does not exist in the container -- but a configured relay forces this key to 1, because the relay cannot run without its state directory. Passive-safe in both directions: a directory confers no reach on its own, and 0 only tightens. The host directory is created unconditionally regardless of this key (since 1.7.0) so directory PRESENCE carries no information; consumers verify the MOUNT (inside: test -d ~/.handoff/relay; outside: docker inspect .Mounts), never the directory or a marker. The operator marker $SANDBOX_DIR/.handoff-enabled (sandbox TOP level, never mounted, survives --reset-sandbox) forces it ON for one sandbox even when config says 0 -- privileged by location like agent-args.<agent>, and a no-op when nothing opts out. Relay state moves to a producer-agnostic directory in a later release (#353), which is what finally retires this key. |
